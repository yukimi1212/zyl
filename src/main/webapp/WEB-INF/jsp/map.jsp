 		...
 		function setAround(point) { 
			...
			local.setMarkersSetCallback(function(pois){
				for(var i=0; i<pois.length; i++){
	                ...
	                var label = new window.BMap.Label("剩余停车位："+100, { offset: new window.BMap.Size(20, -10) });
	                label.setStyle({ color : "gray", fontSize : "12px", backgroundColor: "white", borderColor: "black" });
                    marker.setLabel(label);		
                    marker.addEventListener("dblclick", function(e){
                    	var uurl = "http://localhost:8080/test/open"
		                window.open(uurl);
                    })
	            }	
	        }); 
